# План работ: система управления модулями `modman`
...
Cutover выполнен: `modman` управляет своим реестром и компилирует артефакты конфигурации в
**канонические** пути (DI-конфигурируемо). Прежний патч-modman сохранён как `app/modules/modman_b`
(образец для сверки, нерабочий) и будет удалён после обкатки нового.

Подробности архитектуры — в [ARCHITECTURE.md](./ARCHITECTURE.md).

## Тестовые пакеты

Изначально тестовые пакеты делались с суффиксом `-new` — изоляция на время сосуществования со старым
modman (своё имя/namespace/таблицы/директории). После cutover суффикс не нужен: модули переводятся на
новый контракт под каноническими именами. Принцип конверсии остаётся: модуль реализует
capability-интерфейсы (`DeclaresModule`/`Provides*`) вместо статических методов + `method_exists`,
наследует `CmsModule`, объявляет маркер `extra.bescms`. Ставятся штатно через composer.
...
- [x] Создать каркас `modman` + git-репозиторий
...
## Фаза 11. Обкатка на реальных модулях
- [x] Регистрация modman в приложении (cutover на канонические пути)
- [ ] Перевести пилотные модули на новый контракт (например, shortcode / blog / person)
- [ ] Прогон: scan → check → install → проверка БД/директорий/артефактов → uninstall → reconcile

---

## Журнал решений
- Контракты модуля размещены **внутри** `modman/contract/` для целостности репозитория и удобства
...
- Артефакты компиляции и реестр писались в `@config-dyn-gen` с суффиксом `_new` (параметр
  сосуществования). **Cutover выполнен** — пути переключены на канонические в `config/params.php`.
...
- **Менеджер — обычный модуль.** `Module` реализует `DeclaresModule` + `ProvidesAdminMenu` +
  ...
  (`config/log.php`, категория `modman/*`, как у старого modman). Открывает путь к самообновлению.
...
- **Рантайм-поведение модуля — в тонком базовом классе `common\components\module\CmsModule`**, а не в
  ...
  `modman/Module` тоже на нём (init больше не нужен — namespace и DI-страховку даёт база). Guard на
...
- **Подробный лог установки/удаления — в свой канал `modman/*`** (аналог старого `InstallationLog`).
  Три слоя были дырявы и закрыты: (1) канал поднимается в рантайме из `Bootstrap` (на холодном старте
  артефакт logChannels может быть не собран, иначе всё валилось в общий `monolog.log`); (2)
  ...
  снимает `compact`, захватывает вывод миграций (`create table … done`) и пишет в `modman/migration` —
  это «записи о созданных таблицах». Несоответствия discovery уже шли в `modman/discovery`.
  Файл: `@runtime/logs/monolog-modman.log`.
...
- **Системные модули (editable=false) компилируются ВСЕГДА**, а не только из реестра. `ConfigCompiler`
  раньше брал модули только из реестра (installed); сам менеджер в реестре не значится (бутстрапится
  ядром), поэтому при каждой `recompile` (после install/uninstall) его конфиг и меню вымывались —
  модуль «исчезал» из приложения, а его пункт меню не появлялся. Теперь системные манифесты каталога
  добавляются как всегда-активные (хелпер `addManifest`, дедуп с реестром по id).
