...
- `StandardMigrationHistory` — синхронизация со штатной `{{%migration}}`: менеджер пишет/удаляет записи
  и там тоже. Поэтому миграции, накатанные обычным `yii migrate`, менеджер не запускает повторно (лишь
  закрепляет владельца), а накатанные менеджером видны обычному `yii migrate`. Учёт владения и штатная
  история не расходятся.
